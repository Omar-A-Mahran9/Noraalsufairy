...
class BankOfferController extends Controller
{
    ...
    public function create(){
        ...
        $banks =  Bank::get();
        $sectors =  Sector::get();
        $brands = Brand::select('name_'.getLocale(),'id')->get();

        return view('dashboard.bank_offers.create',compact('banks','brands','sectors'));
    }
    ...
    public function validateRequestData()
    {
        $ValidationArray = [
            ...
            'title_ar'    => 'required | string | max:255 | unique:bank_offers',
            'title_en'    => 'required | string | max:255 | unique:bank_offers',
    ...
    public function store(Request $request){
        ...
        $bankOffer->brands()->attach($data['brand_id']??[]);
    ...
    public function edit(BankOffer $bankOffer){
        $banks =  Bank::get();
        $bankOffer->brands = $bankOffer->brands;
        $bankOffer->sectors = $bankOffer->sectors;
        $selectedBrandIds = $bankOffer->brands->pluck('id')->toArray();
        
        $sectors =  Sector::get();
        $brands = Brand::select('name_'.getLocale(),'id')->get();

        return view('dashboard.bank_offers.edit',compact('banks','brands','sectors','bankOffer','selectedBrandIds'));
    }
    ...
    public function validateRequestUpdateData($id)
    {
        $ValidationArray = [
            ...
            'title_ar'    => 'required | string | max:255 | unique:bank_offers,id,'.$id,
            'title_en'    => 'required | string | max:255 | unique:bank_offers,id,'.$id,
            'description_ar'    => 'required | string ',
            'description_en'    => 'required | string ',
            'from'    => 'required | date',
            'to'    => 'required | date | after:from',
